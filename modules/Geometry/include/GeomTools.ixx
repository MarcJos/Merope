...
inline HalfSpace<DIM> geomTools::faceEquation(
    ...
    TypeFace typeFace) {
    Merope_assert(typeFace != TypeFace::None, "Unexpected");
...
template<unsigned short DIM>
inline double merope::geomTools::fracVolIntersection(
...
void geomTools::HalfSpace_Intersecting_Voxel<DIM, SOLID>::next() {
    if constexpr (is_same_v<Sphere<DIM>, SOLID>) {
        if (counter >= 0) {
            current_half_space.reset();
        } else {
            Point<DIM> normal = centerPoly_to_origVoxel + 0.5 * cubeLength;  // computed wrt to the center of the voxel
            geomTools::renormalize<DIM>(normal);
            current_half_space.reset(new HalfSpace<DIM>(normal, solid.radius));
        }
    }
    //
    else if constexpr (is_same_v<ConvexPolyhedron<DIM>, SOLID>) {
        if (counter + 1 >= solid.faces.size()) {
            current_half_space.reset();
        } else {
            current_half_space.reset(&(solid.faces[counter + 1]), [](auto*) {});
        }
    }
    //
        ...
        if (counter >= 0) {
            ...
            current_half_space.reset(new HalfSpace<DIM>(solid.closestTangentSpace(centerPoly_to_origVoxel + 0.5 * cubeLength)));
        }
    }
    //
    else if constexpr (is_same_v<Cylinder<3>, SOLID>) {
        auto axis_vec = solid.axis[1] - solid.axis[0];
        auto axis_normal = RenormPoint<DIM>(axis_vec).getPoint();
        if (counter >= 2) {
            current_half_space.reset();
        } else if (counter == 1) { // side part of cylinder
            auto centerCyl_to_centerVoxel = centerPoly_to_origVoxel + 0.5 * cubeLength;
            auto lateral_vec =
                (centerCyl_to_centerVoxel - geomTools::prodScal<DIM>(centerCyl_to_centerVoxel, axis_normal) * axis_normal);
            geomTools::renormalize<DIM>(lateral_vec);
            current_half_space.reset(new HalfSpace<DIM>(lateral_vec, solid.radius * lateral_vec));
        } else { // counter = -1 or 0
            current_half_space.reset(new HalfSpace<DIM>(axis_normal, (0.5 + counter) * axis_vec));
        }
    }
    //
    else {
