...
# RSA_CHECK_CXX_SOURCE_COMPILES(<code> <var> [FAIL_REGEX <fail-regex>])
...
#
#  RSA_CMAKE_REQUIRED_FLAGS = string of compile command line flags
#  RSA_CMAKE_REQUIRED_DEFINITIONS = list of macros to define (-DFOO=bar)
#  RSA_CMAKE_REQUIRED_INCLUDES = list of include directories
#  RSA_CMAKE_REQUIRED_LIBRARIES = list of libraries to link
#
# This macro is a copy of CheckCXXSourceCompiles.cmake
# Copyright 2005-2009 Kitware, Inc.
MACRO(RSA_CHECK_CXX_SOURCE_COMPILES SOURCE VAR)
    SET(_FAIL_REGEX)
    SET(_key)
    FOREACH(arg ${ARGN})
    ...
    SET(MACRO_CHECK_FUNCTION_DEFINITIONS
      "-D${VAR} ${RSA_CMAKE_REQUIRED_FLAGS}")
    IF(RSA_CMAKE_REQUIRED_LIBRARIES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_LIBRARIES
        "-DLINK_LIBRARIES:STRING=${RSA_CMAKE_REQUIRED_LIBRARIES}")
    ELSE(RSA_CMAKE_REQUIRED_LIBRARIES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_LIBRARIES)
    ENDIF(RSA_CMAKE_REQUIRED_LIBRARIES)
    IF(RSA_CMAKE_REQUIRED_INCLUDES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_INCLUDES
        "-DINCLUDE_DIRECTORIES:STRING=${RSA_CMAKE_REQUIRED_INCLUDES}")
    ELSE(RSA_CMAKE_REQUIRED_INCLUDES)
      SET(CHECK_CXX_SOURCE_COMPILES_ADD_INCLUDES)
    ENDIF(RSA_CMAKE_REQUIRED_INCLUDES)
    ...
    TRY_COMPILE(${VAR}
      ...
      COMPILE_DEFINITIONS ${RSA_CMAKE_REQUIRED_DEFINITIONS}
    ...
    ELSE(${VAR})
      ...
      FILE(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
        ...
        "Source file was:\n${SOURCE}\n")
    ENDIF(${VAR})
ENDMACRO(RSA_CHECK_CXX_SOURCE_COMPILES)

MACRO (RSA_CHECK_CXX_COMPILER_FLAG _FLAG _RESULT)
   SET(SAFE_RSA_CMAKE_REQUIRED_DEFINITIONS "${RSA_CMAKE_REQUIRED_DEFINITIONS}")
   SET(RSA_CMAKE_REQUIRED_DEFINITIONS "${_FLAG}")
   RSA_CHECK_CXX_SOURCE_COMPILES("int main() { return 0;}" ${_RESULT}
   ...
   SET (RSA_CMAKE_REQUIRED_DEFINITIONS "${SAFE_RSA_CMAKE_REQUIRED_DEFINITIONS}")
ENDMACRO (RSA_CHECK_CXX_COMPILER_FLAG)

MACRO(rsa_enable_cxx_compiler_flag2 out flag var)
  if(MSVC)
	RSA_CHECK_CXX_COMPILER_FLAG("/${flag}" ${var})
  else(MSVC)
	RSA_CHECK_CXX_COMPILER_FLAG("-${flag}" ${var})
  endif(MSVC)
  IF(${var})
  ...
  ENDIF(${var})
ENDMACRO(rsa_enable_cxx_compiler_flag2)

MACRO(rsa_enable_cxx_compiler_flag out)
  IF(${ARGC} LESS 1)
    MESSAGE(FATAL_ERROR "enable_compiler_flag : no flag specified")
  ENDIF(${ARGC} LESS 1)
  FOREACH(f ${ARGN})
    rsa_enable_cxx_compiler_flag2(${out} "${f}" ${f}_AVAILABLE)
  ENDFOREACH(f)
ENDMACRO(rsa_enable_cxx_compiler_flag)
...
#  set(OPTIMISATION_FLAGS "-DNO_RUNTIME_CHECK_BOUNDS ${OPTIMISATION_FLAGS}")
  ...
  # message(FATAL_ERROR "RSA C++11 support is not availabable with the SunPro compiler")
elseif(MSVC)
...
# showing how to build `RSA` with `conda`.
if(NOT USE_EXTERNAL_COMPILER_FLAGS)
