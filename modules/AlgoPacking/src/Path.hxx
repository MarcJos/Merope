...
class Path_auxi {
    Path_auxi(Path_auxi&&) = delete;
    Path_auxi(const Path_auxi&) = delete;
    Path_auxi& operator=(Path_auxi&&) = delete;
    Path_auxi& operator=(const Path_auxi&) = delete;
    Path_auxi();
public:
    static const Path_auxi& get();
    // opposite corners in order to reach get more easily out of the sphere
    template<unsigned short DIM>
    const vector<array<unsigned short, DIM>>& TABCORNER() const;
private:
    const vector<array<unsigned short, 3>> TABCORNER3D;
    const vector<array<unsigned short, 2>> TABCORNER2D;
};
