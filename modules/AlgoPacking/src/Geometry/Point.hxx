...
template<class T, typename std::enable_if_t<is_Point<T, DIM>, bool> = true > \
inline Point<DIM>& operator binop ## = (Point<DIM>& x, const T& y) \
{ sac_de_billes::auxi_function::for_constexpr<0, DIM>([&](auto i){x[i] binop ## = y[i];}); return x; } \
\
inline Point<DIM>& operator binop ## =(Point<DIM>& x, double a)\
{ sac_de_billes::auxi_function::for_constexpr<0, DIM>([&](auto i){x[i] binop ## = a;}); return x; }\
\
...
template<class T1, class T2, typename std::enable_if_t<is_Point<T1, DIM> and is_Point<T2, DIM>, bool> = true > \
inline Point<DIM> operator binop(const T1& x, const T2& y) { Point<DIM> z = x; z binop ## = y; return z; } \
\
template<class T1, typename std::enable_if_t<is_Point<T1, 3>, bool> = true > \
