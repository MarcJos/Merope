...
def plot_tessellation(L, centers, fileVTK):
    ...
    structure = merope.Structure_2D(mi)

    nbVox = [1024 for l in L]
    gridParameters = merope.vox.create_grid_parameters_N_L_2D(nbVox, L)
    grid = merope.vox.GridRepresentation_2D(structure, gridParameters, merope.vox.VoxelRule.Center)
